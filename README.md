...
|---|---|
| `/login` | 学号 + 姓名 登录 |
| `/` 首页 | 本学期总分卡片、最近 5 条加分、最新 3 条活动公示、未读通知红点 |
| `/ranking` | 前 3 名金银铜（匿名）+ 你的排名/百分位 + 全员分数分布 + 全员加分明细透视表（按学号升序，列为活动名） |
| `/announcements` | 活动加分公示：按活动聚合，每个活动展示参与学生姓名 + 分值 |
...
| `/admin/bulk_add` | 批量加分：一个活动一次性给多名学生加同样分值 |
| `/admin/records` | 全部加分记录：搜索、单条撤销 / 恢复、单人手动加分（弹窗） |
...
- **活动公示显示姓名**：「活动公示」页按活动聚合，展示参与学生姓名与分值（不显示学号），作为透明度公示；这是与排名榜匿名原则的**有意差异**——活动名单本身需要公开核对，而总分排名则继续保持匿名。
- **全员加分明细表**：`/ranking` 页下方提供按学号升序的全员透视表（学号 + 姓名 + 各活动分值 + 总和），便于公开核对；与上方"匿名排名榜"共存——匿名看位次，实名看明细。
...
**Q: 加错了分怎么办？**
A: 「加分记录」页搜索 → 点「撤销」（学生会收到通知）。或在批次页面点「整批回滚」撤销整批。撤销后想反悔，可勾选「显示已撤销」再点「恢复」。

**Q: 想临时给某个学生 / 某个活动手动加分（不走文件上传）？**
A: 「加分记录」页右上角「手动加分（单人）」可单条录入；侧边栏「批量加分」可一次性给多名学生加同一活动的同样分值。
...
└── templates/
    ...
    ├── student/{_layout,login,home,me,ranking,announcements}.html
    └── admin/{base,login,dashboard,students,upload,batch_review,
                bulk_add,records,export,admins,logs,settings}.html
```
